...
if command -v ufw &>/dev/null; then
    ...
    if echo "$UFW_OUT" | head -1 | grep -qi "active"; then
        ...
        # Parse UFW rules: extract port numbers from "ALLOW IN" rules
        # Skip Anywhere/v6/etc., focus on the actual port numbers
        UFW_ALLOWED_PORTS=$(echo "$UFW_OUT" \
            | awk '/ALLOW/ && !/DENY|REJECT/ {print $1}' \
            | grep -oE '^[0-9]+(/(tcp|udp))?' \
            | grep -oE '^[0-9]+' \
            | sort -un)
        ...
        if (( ${#ORPHANED[@]} == 0 )); then
            ok "All UFW-allowed ports have active listeners"
