...
if [[ "${1:-}" == "--no-color" ]] || [[ ! -t 1 ]] || [[ "${TERM:-}" == "dumb" ]] || ! tput colors &>/dev/null 2>&1; then
...
# Use a function so we can call it once and pipe it where needed
print_report() {
    echo "+==============================================================+"
    ...
    echo "|  TASK RESULTS                                                |"
    echo "+==============================================================+"
    # Guard against empty array (bash <4.4 errors on expanding empty arrays under set -u)
...
# Print to terminal AND append to log AND save to report file
print_report | tee -a "$LOG_FILE"
print_report | $SUDO tee "$REPORT_FILE" > /dev/null
...
# Build the report into a variable so we write it consistently to terminal,
...
# Save to report file
echo "$REPORT" | $SUDO tee "$REPORT_FILE" > /dev/null
...
# Print to terminal AND save to both report file and main log
echo "$REPORT" | tee -a "$LOG_FILE" | $SUDO tee "$REPORT_FILE" > /dev/null
