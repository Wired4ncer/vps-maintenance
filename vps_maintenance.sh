...
# Pass --no-color in any argument position to force plain output
NO_COLOR=0
for arg in "$@"; do
...
# -- Helpers -------------------------------------------------------------------
# Color vars are stored as literal escape chars ($'...'), so no -e/%b needed --
# use %s to avoid interpreting backslashes in dynamic content (pkg/service names).
log()  { printf '%s\n' "$*" | tee -a "$LOG_FILE"; }
info() { log "${CYAN}  >>${RESET}  $*"; }
...
# tee to a temp file (not a $(...) capture) so output streams live to the
# terminal AND we can both parse the summary and check apt's real exit status.
UPGRADE_TMP=$(mktemp)
DEBIAN_FRONTEND=noninteractive \
$SUDO apt-get upgrade -y \
    -o Dpkg::Options::="--force-confdef" \
    -o Dpkg::Options::="--force-confold" 2>&1 | tee -a "$LOG_FILE" "$UPGRADE_TMP"
UPGRADE_RC=${PIPESTATUS[0]}
UPGRADE_OUT=$(cat "$UPGRADE_TMP"); rm -f "$UPGRADE_TMP"

if (( UPGRADE_RC != 0 )); then
    err "apt-get upgrade failed (exit $UPGRADE_RC) -- check the log above"
    result "Package Upgrades" "XX  FAILED (exit $UPGRADE_RC)"
else
...
DEBIAN_FRONTEND=noninteractive $SUDO apt-get autoremove -y 2>&1 \
    | tee -a "$LOG_FILE" "$AUTOREMOVE_TMP"
AUTOREMOVE_RC=${PIPESTATUS[0]}
AUTOREMOVE_OUT=$(cat "$AUTOREMOVE_TMP"); rm -f "$AUTOREMOVE_TMP"

if (( AUTOREMOVE_RC != 0 )); then
    err "apt-get autoremove failed (exit $AUTOREMOVE_RC) -- check the log above"
    result "Autoremove" "XX  FAILED (exit $AUTOREMOVE_RC)"
else
...
if [[ -n "$ALL_KERNELS" ]]; then
    ...
    # sort -V puts kernels in true version order (alphabetical sort would rank
    # ...-100 before ...-99, risking purge of the newest kernel). Then drop the
    # current kernel and keep the single newest remaining one as a backup.
    OLD_KERNELS=$(echo "$ALL_KERNELS" | grep -vF "$CURRENT_KERNEL" | sort -V | head -n -1 || true)
...
# Failed SSH logins.
# Prefer journalctl: its --since gives a reliable 24h window. Fall back to
# auth.log only when journalctl is unavailable -- but there we count the whole
# current (un-rotated) log rather than faking a time filter, because comparing
# raw log lines against a "%b %e" date is not chronological (month names don't
# sort, ISO timestamps don't match) and silently mis-counts.
info "Checking failed SSH login attempts..."
SSH_WINDOW="last 24h"
...
elif $SUDO test -r /var/log/auth.log; then
    FAILED_SSH=$($SUDO grep -c "Failed password" /var/log/auth.log 2>/dev/null || true)
    FAILED_SSH=${FAILED_SSH:-0}
    SSH_WINDOW="current auth.log"
else
    FAILED_SSH="N/A"
fi
...
if [[ "$FAILED_SSH" == "N/A" ]]; then
    ...
    warn "High number of failed SSH logins ($SSH_WINDOW): $FAILED_SSH"
    result "Failed SSH Logins" "!!  $FAILED_SSH attempts ($SSH_WINDOW)"
else
    ok "Failed SSH logins ($SSH_WINDOW): $FAILED_SSH"
    result "Failed SSH Logins" "OK  $FAILED_SSH attempts ($SSH_WINDOW)"
fi
...
if command -v ufw &>/dev/null; then
    ...
    if echo "$UFW_OUT" | head -1 | grep -qi "active"; then
        ...
        # ----- Check for orphaned UFW rules (allowed ports with no listener) -----
        info "Checking if UFW-allowed ports have active listeners..."

        # Get all listening ports (TCP + UDP) into a single sorted unique list
        if command -v ss &>/dev/null; then
            LISTENING_PORTS=$($SUDO ss -tlnu 2>/dev/null \
                | awk 'NR>1 {print $5}' \
                | grep -oE '[0-9]+$' \
                | sort -un)
        else
            LISTENING_PORTS=$($SUDO netstat -tlnu 2>/dev/null \
                | awk 'NR>2 {print $4}' \
                | grep -oE '[0-9]+$' \
                | sort -un)
        fi

        # Take only inbound ALLOW rules (an "ALLOW OUT" rule is not an open
        # inbound port and must not be flagged). The "To" column is everything
        # before the Action, so strip "ALLOW IN..." onward to isolate it, then
        # drop the trailing "(v6)" marker (the v6 rule just dups the v4 port).
        UFW_INBOUND_TO=$(echo "$UFW_OUT" \
            | grep -E 'ALLOW[[:space:]]+IN' \
            | sed -E 's/[[:space:]]+ALLOW[[:space:]]+IN.*//' \
            | sed -E 's/[[:space:]]*\(v6\)[[:space:]]*$//')

        # Plain numeric port rules (incl. comma lists like 80,443/tcp). Every
        # port is checked, not just the first.
        UFW_PORT_RULES=$(echo "$UFW_INBOUND_TO" \
            | grep -E '^[0-9]+(,[0-9]+)*(/(tcp|udp))?[[:space:]]*$')
        UFW_ALLOWED_PORTS=$(echo "$UFW_PORT_RULES" | grep -oE '[0-9]+' | sort -un)

        # Anything else -- app profiles (OpenSSH, "Nginx Full") and port ranges
        # (1000:2000/tcp) -- can't be mapped to a single listener, so note them
        # rather than mis-flagging them as orphaned.
        UFW_UNVERIFIABLE=$(echo "$UFW_INBOUND_TO" \
            | grep -vE '^[0-9]+(,[0-9]+)*(/(tcp|udp))?[[:space:]]*$' \
            | grep -vE '^[[:space:]]*$' \
            | grep -viE 'Anywhere' \
        ...
        if (( ${#ORPHANED[@]} == 0 )); then
            ok "All UFW-allowed numeric ports have active listeners"
            result "UFW Orphaned Rules" "OK  None"
        else
            warn "UFW has ${#ORPHANED[@]} allowed port(s) with no listener: ${ORPHANED[*]}"
            warn "Consider closing unused rules with: sudo ufw delete allow <port>"
            result "UFW Orphaned Rules" "!!  Ports without listener: ${ORPHANED[*]}"
        fi
        if [[ -n "$UFW_UNVERIFIABLE" ]]; then
            UFW_UNVERIFIABLE_LIST=$(echo "$UFW_UNVERIFIABLE" | tr '\n' ',' | sed 's/,$//')
            info "Not checked (app profiles / port ranges): $UFW_UNVERIFIABLE_LIST"
        fi
    else
...
if [[ -z "$FAILED_SERVICES" ]]; then
    ...
    # The first column is sometimes a status bullet (●), service name is at $1 or $2.
    # Use a more robust parser: pick the field that ends in .service/.socket/.timer/etc.
    FAILED_NAMES=$(echo "$FAILED_SERVICES" \
        | grep -oE '[^[:space:]]+\.(service|socket|timer|target|mount|path|scope|slice|device)' \
        | tr '\n' ' ')
...
if [[ -f /var/run/reboot-required ]]; then
    ...
    if [[ -f /var/run/reboot-required.pkgs ]]; then
        REBOOT_PKGS=$(paste -sd ',' /var/run/reboot-required.pkgs 2>/dev/null | sed 's/,/, /g')
...
# Also check common cert locations if certbot isn't used.
# Note: /etc/ssl/private holds private keys, not certs, so it's intentionally
# excluded; /etc/ssl/certs/*.pem covers manually installed certs.
if [[ $CERT_COUNT -eq 0 ]] && command -v openssl &>/dev/null; then
    for CERT_PATH in /etc/nginx/ssl/*.crt /etc/nginx/ssl/*.pem \
                     /etc/apache2/ssl/*.crt /etc/ssl/certs/*.pem; do
