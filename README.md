...
- The sudo keepalive runs fully detached from the terminal (`</dev/null >/dev/null`) so it cannot interfere with stdin.# vps_maintenance.sh
...
```bash
ls -lt /var/log/vps_maintenance/ | head -5
cat /var/log/vps_maintenance/report_<timestamp>.txt
```
